...
func TestFilterIntersectsExistingSelection(t *testing.T) {
	...
	filter := &Filter{
		Predicate:  storage.NewPredicateEvaluator(storage.Predicate{Column: "amount", Op: storage.PredicateOpEq, Int64: 42}),
...
func TestScanFilterProjectLimitCountPipeline(t *testing.T) {
	...
	filter := &Filter{Predicate: storage.NewPredicateEvaluator(storage.Predicate{Column: "amount", Op: storage.PredicateOpEq, Int64: 42}), Downstream: project}
