...
func TestDeltaBitpack_RoundTrip(t *testing.T) {
	cases := []struct {
		...
		{"MonotonicInt64", vector.VecInt64, 2048,
			func(v vector.Vec) {
		...
		{"Timestamp", vector.VecTimestamp, 100,
			func(v vector.Vec) {
...
func TestDeltaBitpack_EstimateRejectsNonFORKinds(t *testing.T) {
	for _, kind := range []vector.VecKind{vector.VecBool, vector.VecFloat32, vector.VecText, vector.VecUUID} {
		var v vector.Vec
...
func TestDeltaBitpack_DecodeRejects(t *testing.T) {
	cases := []struct {
		...
		{"truncatedHeader", make([]byte, 10), vector.VecInt64, 4},
		{"rowsLessThan2", make([]byte, deltaHeaderSize), vector.VecInt64, 1},
		{"negativeRows", nil, vector.VecInt64, -1},
		{"nonFORKind", make([]byte, deltaHeaderSize+128), vector.VecText, 2},
	}
	for _, tc := range cases {
		var dst vector.Vec
...
func TestDeltaBitpack_DecodeRangeChecksInt32(t *testing.T) {
	src := vector.NewVec(vector.VecInt32, 3)
	...
	var dst vector.Vec
	if err := (deltaBitpackCodec{}).Decode(payload, vector.VecInt32, 3, 0, &dst); err == nil {
