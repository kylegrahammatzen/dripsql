...
func TestConstant_Int64RoundTrip(t *testing.T) {
	...
	var dst types.Vec
	if err := (constantCodec{}).Decode(payload, types.VecInt64, 100, 0, &dst); err != nil {
...
func TestConstant_BoolAllTrueAllFalse(t *testing.T) {
	for _, allTrue := range []bool{true, false} {
		...
		var dst types.Vec
		if err := (constantCodec{}).Decode(payload, types.VecBool, 17, 0, &dst); err != nil {
...
func TestConstant_VarBytesRoundTrip(t *testing.T) {
	src := types.NewVarVec(types.VecText, 50, 0)
	vb := src.Var()
	for i := range 50 {
	...
	var dst types.Vec
	if err := (constantCodec{}).Decode(payload, types.VecText, 50, 0, &dst); err != nil {
		t.Fatalf("Decode: %v", err)
	}
	for i := range 50 {
...
func TestConstant_VarBytesBroadcastIsCompact(t *testing.T) {
	...
	if err := (constantCodec{}).Decode(payload, types.VecText, 1000, 0, &dst); err != nil {
		t.Fatalf("Decode: %v", err)
	}
...
func TestConstant_DecodeRejectsTrailingBytes(t *testing.T) {
	var dst types.Vec
	if err := (constantCodec{}).Decode(make([]byte, 9), types.VecInt64, 4, 0, &dst); err == nil {
		t.Fatal("constant decode must reject trailing bytes on fixed kind")
	}
}

func TestConstant_EmptyVec(t *testing.T) {
	src := types.NewVec(types.VecInt32, 0)
	payload, err := constantCodec{}.Encode(src, nil)
	if err != nil {
	...
	var dst types.Vec
	if err := (constantCodec{}).Decode(payload, types.VecInt32, 0, 0, &dst); err != nil {
...
func TestConstant_DecodeRejectsTruncated(t *testing.T) {
	var dst types.Vec
	if err := (constantCodec{}).Decode([]byte{0}, types.VecInt64, 4, 0, &dst); err == nil {
		t.Fatal("fixed Constant decode must reject payload shorter than width")
	}
	if err := (constantCodec{}).Decode([]byte{}, types.VecBool, 4, 0, &dst); err == nil {
		t.Fatal("bool Constant decode must reject empty payload")
	}
	if err := (constantCodec{}).Decode([]byte{0, 0, 0}, types.VecText, 1, 0, &dst); err == nil {
		t.Fatal("varbytes Constant decode must reject header truncated")
	}
	if err := (constantCodec{}).Decode([]byte{10, 0, 0, 0, 'h'}, types.VecText, 1, 0, &dst); err == nil {
...
func TestConstant_EstimateMatchesEncode(t *testing.T) {
	src := types.NewVarVec(types.VecText, 8, 0)
	for i := range 8 {
...
func TestConstant_DecodeRejectsNonCanonicalBool(t *testing.T) {
	var dst types.Vec
	if err := (constantCodec{}).Decode([]byte{2}, types.VecBool, 10, 0, &dst); err == nil {
		t.Fatal("constant bool decode must reject non-canonical payload byte")
	}
	if err := (constantCodec{}).Decode([]byte{0xFF}, types.VecBool, 10, 0, &dst); err == nil {
		t.Fatal("constant bool decode must reject 0xFF")
	}
	if err := (constantCodec{}).Decode([]byte{0}, types.VecBool, 10, 0, &dst); err != nil {
		t.Fatalf("constant bool decode must accept 0: %v", err)
	}
	if err := (constantCodec{}).Decode([]byte{1}, types.VecBool, 10, 0, &dst); err != nil {
