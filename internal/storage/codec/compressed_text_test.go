...
func TestCompressedText_LookupBoth(t *testing.T) {
	for _, enc := range []types.Encoding{types.EncodingFlate, types.EncodingZstd} {
		c, err := Lookup(enc)
		if err != nil {
...
func TestCompressedText_RoundTripBothEncodings(t *testing.T) {
	for _, enc := range []types.Encoding{types.EncodingFlate, types.EncodingZstd} {
		t.Run(enc.String(), func(t *testing.T) {
			src := types.NewVarVec(types.VecText, 5, 0)
			vb := src.Var()
			vb.AppendString(0, "")
			...
			var dst types.Vec
			if err := c.Decode(payload, types.VecText, 5, 0, &dst); err != nil {
...
func TestCompressedText_CompressesRepetitiveInput(t *testing.T) {
	for _, enc := range []types.Encoding{types.EncodingFlate, types.EncodingZstd} {
		t.Run(enc.String(), func(t *testing.T) {
			src := types.NewVarVec(types.VecText, 1000, 0)
			vb := src.Var()
			for i := range 1000 {
...
func TestCompressedText_DecodeRejectsTruncatedHeader(t *testing.T) {
	c, _ := Lookup(types.EncodingFlate)
	var dst types.Vec
	if err := c.Decode([]byte{0, 0}, types.VecText, 1, 0, &dst); err == nil {
...
func TestCompressedText_DecodeRejectsLengthMismatch(t *testing.T) {
	...
	payload[1] = 0xFF
	var dst types.Vec
	if err := c.Decode(payload, types.VecText, 3, 0, &dst); err == nil {
...
func TestCompressedText_DecodeSetsEncFlat(t *testing.T) {
	...
	for _, enc := range []types.Encoding{types.EncodingFlate, types.EncodingZstd} {
		...
		var dst types.Vec
		if err := c.Decode(payload, types.VecText, 4, 0, &dst); err != nil {
